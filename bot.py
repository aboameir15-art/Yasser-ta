...
async def get_active_trades_report(user_id):
    ...
    for trade in trades:
        ...
        # تحويل القيم لـ int (نظام BigInt)
        ...
        # جلب السعر الحالي من جدول المحاكاة
        ...
        # إظهار TP و SL (تظهر فقط إذا كانت محددة في الإعدادات/القاعدة)
        if tp: report_text += f"• جـني الأرباح (TP): <code>{int(tp):,}</code>\n"
        if sl: report_text += f"• وقـف الخسارة (SL): <code>{int(sl):,}</code>\n"
        
        # سعر التصفية (Liquidation)
...
def get_trades_keyboard(user_id, trades):
    ...
    for trade in trades:
        ...
        markup.row(
            ...
            InlineKeyboardButton(f"❌ إغلاق الصفقة", callback_data=f"pre_close:{t_id_str}")
        )
...
async def handle_dca_by_percent(callback_query: types.CallbackQuery):
    ...
    # 1. جلب بيانات الصفقة ورصيد البنك
    res = supabase.table("active_trades").select("*").eq("trade_id", t_id).execute()
    user_res = supabase.table("users_global_profile").select("bank_balance").eq("user_id", user_id).execute()
    
    if not res.data or not user_res.data:
        return await callback_query.answer("⚠️ حدث خطأ في جلب البيانات.")

    trade = res.data[0]
    bank_balance = int(user_res.data[0]['bank_balance'])
    
    # 2. حساب مبلغ التعزيز بناءً على نسبة من الرصيد الحالي
    dca_amount = int(bank_balance * (percent / 100))
    
    if dca_amount < 1: # حد أدنى للتعزيز
        return await callback_query.answer("❌ رصيدك قليل جداً لهذا الإجراء.", show_alert=True)

    # 3. جلب السعر الحالي للعملة
    coin_res = supabase.table("crypto_market_simulation").select("current_price").eq("symbol", trade['symbol']).execute()
    current_price = int(coin_res.data[0]['current_price'])

    # --- معادلات بينانس لمتوسط السعر (Averaging) ---
    old_margin = int(trade['margin'])
    old_entry = int(trade['entry_price'])
    lev = int(trade['leverage'])
    side = trade['side']

    # حساب الوحدات (الكمية) القديمة والجديدة
    old_units = (old_margin * lev) / old_entry
    new_units_added = (dca_amount * lev) / current_price
    total_units = old_units + new_units_added
    
    # السعر المتوسط الجديد = (إجمالي الهامش * الرافعة) / إجمالي الوحدات
    new_margin = old_margin + dca_amount
    new_entry_price = int((new_margin * lev) / total_units)
    
    # حساب سعر التصفية الجديد (استخدم دالتك Calculate Liquidation)
    new_liq = int(calculate_liquidation(new_entry_price, lev, side))

    # 4. التحديث في سوبابيس
    # خصم من البنك
    supabase.table("users_global_profile").update({"bank_balance": bank_balance - dca_amount}).eq("user_id", user_id).execute()
    ...
    supabase.table("active_trades").update({
        ...
        "quantity": int(total_units) # تحديث الكمية أيضاً
    }).eq("trade_id", t_id).execute()

    await callback_query.answer(f"🚀 تم تعزيز الصفقة بمبلغ {dca_amount:,} $\nالسعر الجديد: {new_entry_price:,}", show_alert=True)
    await callback_view_trades(callback_query)
    
@dp.callback_query_handler(Text(startswith='p_close:'))
async def handle_partial_close(callback_query: types.CallbackQuery):
    data = callback_query.data.split(':')
    percent = int(data[1])
    t_id = data[2]
    user_id = int(callback_query.from_user.id)
    ...
    trade = res.data[0]
    if percent == 100:
        # استدعاء دالة الإغلاق الكلي التي برمجناها سابقاً
        symbol = trade['symbol']
        coin_res = supabase.table("crypto_market_simulation").select("current_price").eq("symbol", symbol).execute()
        current_price = int(coin_res.data[0]['current_price'])
        await close_trade_manually(t_id, current_price)
        await callback_query.answer("✅ تم إغلاق الصفقة بالكامل.", show_alert=True)
    else:
        # --- إغلاق جزئي احترافي ---
        old_margin = int(trade['margin'])
        old_entry = int(trade['entry_price'])
        ...
        # جلب السعر الحالي لحساب الربح
        coin_res = supabase.table("crypto_market_simulation").select("current_price").eq("symbol", trade['symbol']).execute()
        current_price = int(coin_res.data[0]['current_price'])

        # 1. حساب الربح/الخسارة للجزء المراد إغلاقه
        ...
        # المبلغ العائد للبنك = الهامش المغلق + ربحه (أو - خسارته)
        return_to_bank = margin_to_close + pnl_amount
        if return_to_bank < 0: return_to_bank = 0 # حماية من الرصيد السالب
        ...
        # 3. تحديث الصفقة (تقليل الهامش والكمية)
        ...
        await callback_query.answer(f"✅ تم إغلاق {percent}% من المركز.\nتم إعادة {return_to_bank:,} $ للمحفظة.", show_alert=True)

    await callback_view_trades(callback_query)
    
@dp.callback_query_handler(Text(startswith='pre_close:'), state="*")
async def pre_close_confirmation(callback_query: types.CallbackQuery):
    await callback_query.answer()
    
    # فك البيانات: pre_close:user_id:trade_id
    data = callback_query.data.split(':')
    user_id = data[1]
    t_id = data[2]
    
    # جلب اسم العملة لعرضه في رسالة التأكيد
    res = supabase.table("active_trades").select("symbol").eq("trade_id", t_id).execute()
    if not res.data:
        return await callback_query.message.edit_text("⚠️ الصفقة مغلقة بالفعل.")
    
    symbol = res.data[0]['symbol']
    
    text = f"⚠️ <b>تأكيـد إغلاق الصـفقة</b>\n\n"
    text += f"هل أنت متأكد من رغبتك في إغلاق مركز <b>#{symbol}</b> فوراً بسعر السوق؟"
    
    markup = InlineKeyboardMarkup()
    markup.row(
        # هذا الزر سينقل المستخدم للدالة التي أرسلتها أنت (التنفيذ الفعلي)
        InlineKeyboardButton("✅ نعم، أغلق الآن", callback_data=f"close_confirm:{user_id}:{t_id}"),
        # زر التراجع يعيده لشاشة الإعدادات
        InlineKeyboardButton("❌ تراجع", callback_data=f"manage_trade:{t_id}")
    )
    
    await callback_query.message.edit_text(text, reply_markup=markup, parse_mode="HTML")
    
@dp.callback_query_handler(Text(startswith='close_confirm:'), state="*")
async def handle_manual_close_request(callback_query: types.CallbackQuery):
    if not await is_authorized(callback_query): return
    
    # إيقاف التحميل
    await callback_query.answer("جاري تنفيذ الإغلاق...", show_alert=False)
    ...
    # 3. استدعاء دالة الإغلاق (التي تقوم بالحسابات والخصم من البنك)
    ...
    if success:
        ...
        # العودة لعرض قائمة الصفقات المحدثة
        await callback_view_trades(callback_query) 
    else:
